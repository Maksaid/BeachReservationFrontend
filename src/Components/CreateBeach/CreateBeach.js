    ...
    const handleSubmitCreation = async (e) => {
        ...
        if (beachName === "" && beachDescription === "") {
            ...
            try {
                ...
                (error) {
                console.error(error);
                error.response.status === 401 ? setSuccessMessage("Login please, your session is expired") : setSuccessMessage("Beach-creation gone wrong");
            }
    }
    ;
    const sendBackgroundImage = async (beachId, e) => {
        ...
        try {
            if (image) {
                ...
                setCurrRows(2);
                setCurrColumns(4);
                setSuccessMessage("Success! Beach added!")
            } else {
